...
// Lead fields come from visitor input via the agent — escape before embedding in email HTML.
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `New lead — ${(lead.name ?? 'Anonymous').replace(/[\r\n]+/g, ' ')} · ${lead.tier.replace(/[\r\n]+/g, ' ')} tier`,
        html: `
          <div style="font-family:monospace;max-width:560px">
            <h2 style="margin-bottom:20px">New Lead — LYVECA AI Agent</h2>
            <table style="border-collapse:collapse;width:100%;font-size:13px">
              <tr><td style="padding:8px 12px;border:1px solid #ddd;color:#888">Name</td><td style="padding:8px 12px;border:1px solid #ddd">${esc(lead.name)}</td></tr>
              <tr><td style="padding:8px 12px;border:1px solid #ddd;color:#888">Email</td><td style="padding:8px 12px;border:1px solid #ddd">${esc(lead.email)}</td></tr>
              <tr><td style="padding:8px 12px;border:1px solid #ddd;color:#888">Company</td><td style="padding:8px 12px;border:1px solid #ddd">${esc(lead.company)}</td></tr>
              <tr><td style="padding:8px 12px;border:1px solid #ddd;color:#888">Project</td><td style="padding:8px 12px;border:1px solid #ddd">${esc(lead.project_type)}</td></tr>
              <tr><td style="padding:8px 12px;border:1px solid #ddd;color:#888">Tier match</td><td style="padding:8px 12px;border:1px solid #ddd">${esc(lead.tier)}</td></tr>
              <tr><td style="padding:8px 12px;border:1px solid #ddd;color:#888">Budget</td><td style="padding:8px 12px;border:1px solid #ddd">${esc(lead.budget)}</td></tr>
              <tr><td style="padding:8px 12px;border:1px solid #ddd;color:#888">Notes</td><td style="padding:8px 12px;border:1px solid #ddd">${esc(lead.notes)}</td></tr>
